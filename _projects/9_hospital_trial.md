...
Hospitals face a growing shortage of nursing staff, and patient-handling tasks — fetching items and helping patients walk — are a leading source of caregiver strain and injury. ARNA (Adaptive Robotic Nursing Assistant) was built to take on those two tasks, but a robot only helps if the people at the bedside will actually accept it. **The goal of this trial was to measure that acceptance directly — with real patients and nurses, inside an active hospital rather than a lab.**

I designed and led the study around two clinical scenarios. In the **Patient Sitter** scenario, ten patients teleoperated ARNA's arm to fetch and deliver objects, using a touchscreen tablet in one trial and a joystick in another so the two interfaces could be compared head-to-head. In the **Nurse Walker** scenario, five nurses walked with ARNA as a shared-control robotic walker driven by a neuroadaptive force/torque controller. I instrumented the robot to log objective task-completion times and collected Technology Acceptance Model (TAM) surveys — perceived usefulness, ease of use, behavioral intention, and satisfaction — then analyzed the data with linear mixed-effects models in R.
...
- The **joystick was the better patient interface** — significantly easier to use and more likely to be adopted (p ≈ 0.02–0.03) — even though objective task times between the two were statistically similar.
- **Prior video-game experience was a strong predictor of acceptance**, a concrete signal that onboarding and training matter, especially for older patients (mean patient age ≈ 64).
- The core TAM chain held with high explanatory power — ease of use drove perceived usefulness, which drove intention to use — confirming that both patients and nurses saw ARNA as genuinely useful, not just novel.
...
Taking the robot onto a working hospital unit — with real patients and nurses, not a lab surrogate — is what makes this evaluation unusual. Full methods and statistics are in the paper {% cite sharafianardakani2025hospital %}.
