...
- The harder the path, the bigger the gain. The interaction terms were larger than the main effect, at $$\beta = -66.97$$ ($$p = 0.001$$) on the obstacle course and $$\beta = -84.95$$ ($$p < .001$$) on the square path. Turns and avoidance are where a fixed admittance model struggles most, and they are also where estimating intent pays off.
- Stability is proven rather than only observed. The weight-update laws come out of a Lyapunov analysis that bounds the tracking error, so the adaptation stays well-behaved as long as the user's own intent is smooth and bounded.
