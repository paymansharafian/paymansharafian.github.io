(function () {
  ...
  function fabrik(target, iters) {
    ...
    for (let k = 0; k < iters; k++) {
      joints[N - 1].x = target.x;
      joints[N - 1].y = target.y;
      for (let i = N - 2; i >= 0; i--) {
  ...
  function drawMobileBase() {
    ...
    ctx.lineWidth = 0.6;
    const tMidX1 = (p.ftl.x + p.ftr.x) / 2,
      tMidY1 = (p.ftl.y + p.ftr.y) / 2;
    const tMidX2 = (p.btl.x + p.btr.x) / 2,
      tMidY2 = (p.btl.y + p.btr.y) / 2;
    ctx.beginPath();
    ctx.moveTo(tMidX1, tMidY1);
    ctx.lineTo(tMidX2, tMidY2);
    ctx.stroke();
    const tMidX3 = (p.ftl.x + p.btl.x) / 2,
      tMidY3 = (p.ftl.y + p.btl.y) / 2;
    const tMidX4 = (p.ftr.x + p.btr.x) / 2,
      tMidY4 = (p.ftr.y + p.btr.y) / 2;
    ctx.beginPath();
    ctx.moveTo(tMidX3, tMidY3);
    ctx.lineTo(tMidX4, tMidY4);
    ctx.stroke();
  ...
  function drawGripper(x, y, angle, openAmt) {
    ...
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(-gap - 2 * s, 0);
    ctx.lineTo(gap + 2 * s, 0);
    ctx.stroke();
    noGlow();
    ctx.restore();
  }
